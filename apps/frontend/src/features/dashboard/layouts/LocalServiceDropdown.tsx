...
const LocalServiceDropdown = ({
  ...
  const selectedService = useMemo(() => {
    ...
    return (
      services.find((service) => (service._id || service.id) === value) ?? null
    );
    ...
    <div ref={menuRef} className="relative">
      ...
      <button
        ...
        disabled={
          ...
          servicesLoading ||
          servicesError ||
          (!includeAllOption && services.length === 0)
        }
      ...
      {isOpen && (
        ...
        >
          {servicesLoading ? (
            <div className="px-2 py-2 text-[0.625rem] text-body">
              Loading services...
            </div>
          ) : servicesError ? (
            <div className="px-2 py-2 text-[0.625rem] text-red-300">
      ...
      {servicesError && !isOpen && (
        <p className="mt-1 text-[0.625rem] text-red-300">
          Could not load services.
        </p>
      )}
    </div>
