...
export const registerOrganization = asyncHandler(async (req, res, next) => {
  ...
  if (orgExists) {
    return next(
      new ApiError(404, "Organization already exists with this email"),
    );
  }
  ...
  const verificationUrl = `http://localhost:4200/incidentwatch/verify/email/${refreshToken}`;
...
export const verifyEmail = asyncHandler(async (req, res, next) => {
  ...
  if (!updateuser && !updateorg)
    return next(new ApiError(400, "Account not verified."));
