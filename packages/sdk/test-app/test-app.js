...
app.get("/error-rejection", (req, res) => {
  ...
  Promise.reject(new Error("Unhandled promise rejection — /error-rejection"));
  res.json({
...
app.get("/slow-operation", async (req, res) => {
  log.info("[TEST] /slow-operation — 2s delay with startTimer, expect SEV3");
...
app.get("/axios-network", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Axios call failed", message: err.message });
    return;
  }
...
app.get("/axios-500", async (req, res) => {
  log.info("[TEST] /axios-500 — mock 500 endpoint, expect SEV2 upstream error");
  try {
    await axios.get("http://localhost:5000/api/fake-500", { timeout: 5000 });
  } catch (err) {
...
app.get("/ignored-error", async (req, res) => {
  log.info("[TEST] /ignored-error — ECONNRESET, expect NO incident (filtered)");
...
app.get("/set-tag-dynamic", async (req, res) => {
  ...
  await iw.captureMessage("Incident with dynamic tag", { severity: "SEV3" });
...
app.get("/wrap-async", async (req, res) => {
  ...
  res.json({ message: "Wrapped async error captured by SDK automatically" });
});
...
app.listen(PORT, () => {
  ...
  console.log(`  SDK → http://localhost:5000`);
